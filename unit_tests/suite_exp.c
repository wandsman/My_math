...
START_TEST(exp_0) {
  double x = 0;
  ck_assert_int_eq(my_exp(x), exp(x));
}
END_TEST
...
START_TEST(exp_17) { ck_assert_float_eq(my_exp(2), exp(2)); }
END_TEST

START_TEST(exp_18) { ck_assert_float_eq(my_exp(-750), exp(-750)); }
END_TEST

START_TEST(exp_19) { ck_assert_float_eq(my_exp(-2), exp(-2)); }
END_TEST
...
START_TEST(exp_23) {
  double x = 0.0;
  ck_assert_ldouble_eq_tol(my_exp(x), expl(x), 1e-06);
}
END_TEST

START_TEST(exp_24) {
  ck_assert_ldouble_eq_tol(my_exp(-INFINITY), expl(-INFINITY), 1e-06);
}
END_TEST
