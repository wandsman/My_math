...
START_TEST(sin_2) { ck_assert_float_eq(my_sin(0), sin(0)); }
END_TEST
...
START_TEST(sin_7) {
  double num = NAN;
  long double orig_res = sin(num), our_res = my_sin(num);
  int suc = 0;
...
START_TEST(sin_14) { ck_assert_double_eq_tol(my_sin(0.0), sin(0.0), 1e-06); }
END_TEST
