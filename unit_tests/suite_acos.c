...
START_TEST(acos_6) {
  double num = NAN;
  long double orig_res = my_acos(num), our_res = acos(num);
  int suc = 0;
  if (isnan(orig_res) && isnan(our_res)) suc = 1;
  ck_assert_int_eq(1, suc);
} END_TEST
...
START_TEST(acos_7) {
    ck_assert_ldouble_nan(my_acos(NAN));
...
START_TEST(acos_17) {
    ck_assert_ldouble_eq_tol(my_acos(2 / 2), acos(2 / 2), my_EPS);
} END_TEST
