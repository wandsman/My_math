...
START_TEST(atan_1) { ck_assert_float_eq(my_atan(-15.01), atan(-15.01)); }
END_TEST
...
START_TEST(atan_4) {
  ck_assert_ldouble_eq(my_atan(INFINITY), atan(INFINITY));
  ck_assert_ldouble_eq(my_atan(-INFINITY), atan(-INFINITY));
}
END_TEST
...
START_TEST(atan_6) {
  ck_assert_ldouble_eq_tol(my_atan(INFINITY), (M_PI / 2.0), my_EPS);
  ck_assert_ldouble_eq_tol(atan(INFINITY), (M_PI / 2.0), my_EPS);
  ck_assert_ldouble_eq_tol(my_atan((-INFINITY)), ((M_PI) / -2.0), my_EPS);
  ck_assert_ldouble_eq_tol(atan(-INFINITY), ((M_PI) / -2.0), my_EPS);
}
END_TEST
...
START_TEST(atan_15) {
  ck_assert_ldouble_eq_tol(my_atan(2 / 2), atan(2 / 2), my_EPS);
}
END_TEST
...
START_TEST(atan_17) { ck_assert_ldouble_eq_tol(my_atan(1), atan(1), my_EPS); }
END_TEST
...
START_TEST(atan_22) { ck_assert_float_nan(my_atan(my_NAN)); }
END_TEST

START_TEST(atan_23) {
  ck_assert_ldouble_eq_tol(my_atan(my_PI / 2), atan(my_PI / 2), my_EPS);
}
END_TEST
