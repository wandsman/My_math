...
START_TEST(cos_2) {
  double num = -0.999999;
  long double orig_res = cos(num), our_res = my_cos(num);
  int suc = 0;
  if ((fabsl(orig_res - our_res) <= 1e-6)) suc = 1;
  ck_assert_int_eq(1, suc);
}
...
START_TEST(cos_4) { ck_assert_float_eq(my_cos(0), cos(0)); }
END_TEST

START_TEST(cos_5) { ck_assert_float_eq(my_cos(-1), cos(-1)); }
END_TEST
...
START_TEST(cos_10) {
  ck_assert_ldouble_nan(my_cos(-INFINITY));
...
START_TEST(cos_12) {
  ck_assert_ldouble_nan(my_cos(NAN));
  ck_assert_ldouble_nan(cosl(NAN));
}
END_TEST

START_TEST(cos_13) { ck_assert_double_eq_tol(my_cos(0.0), cosl(0.0), 1e-06); }
END_TEST
