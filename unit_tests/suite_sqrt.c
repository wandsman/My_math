...
START_TEST(sqrt_1) {
    double num = INFINITY;
    long double orig_res = sqrt(num), our_res = my_sqrt(num);
    int suc = 0;
...
START_TEST(sqrt_3) {
    double num = -15.01;
    ck_assert_ldouble_nan(my_sqrt(num));
...
START_TEST(sqrt_6) {
    ck_assert_ldouble_infinite(my_sqrt(INFINITY));
...
START_TEST(sqrt_13) {
    ck_assert_ldouble_eq_tol(my_sqrt(100), sqrt(100), 1e-6);
} END_TEST
...
START_TEST(sqrt_16) {
    ck_assert_ldouble_eq_tol(my_sqrt(981.123e-15), sqrt(981.123e-15), 1e-6);
} END_TEST

  Suite * suite_sqrt(void) {
    Suite *s14;
    TCase *tc14;
    s14 = suite_create("my_sqrt");
