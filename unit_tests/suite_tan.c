...
START_TEST(tan_1) { ck_assert_float_eq(my_tan(-15.01), tan(-15.01)); }
END_TEST
...
START_TEST(tan_4) {
  double num = NAN;
  long double orig_res = tan(num), our_res = my_tan(num);
  int suc = 0;
...
START_TEST(tan_8) {
  ck_assert_ldouble_nan(my_tan(-INFINITY));
...
START_TEST(tan_10) {
  ck_assert_ldouble_nan(my_tan(NAN));
  ck_assert_ldouble_nan(tanl(NAN));
}
END_TEST

START_TEST(tan_11) { ck_assert_double_eq_tol(my_tan(0.0), tanl(0.0), 1e-06); }
END_TEST
...
START_TEST(tan_20) {
  ck_assert_double_eq_tol(my_tan(my_PI / 2), tanl(my_PI / 2), 1e-06);
}
END_TEST
